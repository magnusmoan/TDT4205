...
void translate(Translator* translator, int c){
	...
	switch(c){
		case ' ' :
			if(translator->currNode->label == ' '){
				putchar(c);
			}else{
				putchar(translator->currNode->label);
				translator->currNode = translator->tree;
			}
			break;

		case '.' :
			translator->currNode = translator->currNode->dot;
			break;

		case '-' :
			translator->currNode = translator->currNode->dash;
			break;
	}
}
