...
variable_list	      : decleration_statement { $$ = CN(variable_list_n,1,$1); }
		      | variable_list ',' decleration_statement { $$ = CN(variable_list_n,2,$1,$3); }
		      ;
...
statement	      : decleration_statement ';' { $$ = CN(statement_n,1,$1); }
...
decleration_statement : type variable { $$ = CN(declaration_statement_n,2,$1,$2); }
		      ;
...
index		      : INT_CONST { $$ = CNT(index_n,INT_TYPE,0);
				    SetInteger($$,STRDUP(yytext)); }
		      ;
